...
const CountryFlagValidator: React.FC<Props> = ({ password }) => {
    ...
    return (
        <div>
            <p>Heslo musí obsahovat zkratku země:</p>
            <img
                src={flagUrl}
                alt={`Vlajka ${selectedCountry}`}
                style={{ width: 80, borderRadius: 4, marginBottom: 8 }}
            />
            <p>
                Zkratka: <strong>{selectedCountry}</strong>
            </p>
            <p>
                Validace:{" "}
                <strong>{isValid ? "Splněno" : `Heslo neobsahuje zkratku: ${selectedCountry}`}</strong>
            </p>
        </div>
    );
};
